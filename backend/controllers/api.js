...
async function getComments(videoId) {
  ...
  do {
    ...
    try {
      ...
      console.log('Youtube down')
      throw new Error(error)
    }
    nextPageToken = response.data['nextPageToken']
    commentPage = response.data['items'].map(
  ...
  let commentsTimed = allComments.filter((comment) =>
    Boolean(comment['timestamps'])
  )
  ...
  if (video) {
    ...
    commentsTimed.forEach((comment) => {
      prev = prevComments.find(
        (prevComment) => prevComment.text == comment.text
      )
